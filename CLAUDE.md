...
a dedicated small crop at `assets/mascot-mark.png` (64×64, downscaled from
`assets/apple-touch-icon.png`). Alex explicitly asked for this on
2026-09-01, reversing the earlier "tab-icon only" decision below — the
mascot is still not part of the founder/guide content itself (no chatbot
persona, no illustrated character replacing Alex), just the brand mark.
The browser-tab favicon (`favicon.ico` + `assets/favicon-*.png` +
`assets/apple-touch-icon.png`, linked from every page's `<head>`) also
uses the same mascot (`social/aits-mascot.png`).
...
- **Buffer, Later, and other paid social-scheduling APIs are still ruled
  ...
  Every feed post is a **3-slide carousel** (English, Portuguese, Spanish —
  see `social/carousels/`, generated by `scripts/build_social_carousels.py`
  from hand-authored per-language text, not machine translation), added
  2026-09-01 at Alex's request: Instagram can't adapt a single post's
  content per viewer the way the website's language switcher does, so
  swiping between slides is the Instagram-native equivalent. The homepage's
  own preview of these posts (`index.html`, `#social`) uses the same
  per-language PNGs and swaps them live when the site's language switcher
  changes, for the same reason. `social/content-calendar.md` stays as the
  human-readable version of the same schedule. The 3 Reels are still posted
  by hand — no video file
...
- **The AI News page (`ai-news.html`) is a second instance of the same
  GitHub-Actions-as-backend pattern**, added 2026-09-01 at Alex's explicit
  request to stop being a hand-maintained snapshot. This is the one place
  in the project that does call a paid, key-authenticated API (Anthropic's
  own Messages API) — deliberately, and only because summarizing/rewriting
  headlines into original text is genuinely a language-model job, unlike
  the templated Instagram captions. The key
  (`ANTHROPIC_API_KEY`) is a GitHub Actions secret Alex enters himself —
  never handled by Claude, exactly like `IG_ACCESS_TOKEN` above. Pipeline:
  ...
  daily and commits the file back if it changed. Full one-time setup is in
  `news/ai-news-setup.md`. Outbound source links are intentionally
...
AITS/
  ...
  social/
    ...
    carousels/                    # EN/PT/ES carousel slides for all 9 posts (SVG + PNG), from build_social_carousels.py
    meta-business-suite-guide.md  # how Alex posts by hand via Meta's free scheduling dashboard (no API key)
